//Implementacion vectorial arbol binario.
...
public:
	...
	void eliminarRaizB();
	//void AlturaNodo(nodo n)const;	//Implementada por mi.
	//int Max(int a, int b);
	~Abin(); // destructor
...
template <typename T>
void Abin<T>::AlturaNodo(Abin<T>::nodo n)const{
	if(Abin<T>::nodo n==Abin<T>::NODO_NULO){
		return -1;
	}else{
		1+Max(AlturaNodo(nodos[n].hder),AlturaNodo(nodos[n].hizq))
	}
}
