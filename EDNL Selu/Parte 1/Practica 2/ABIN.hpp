...
private:
	...
	nodo copiar(nodo n);
	//Mis funciones:
	int alturaBrec(nodo n,const Abin<T>& A);
	int Max(int a,int b);
};
...
inline void Abin<T>::eliminarRaizB(){
	...
	assert(r->hizq == NODO_NULO && r->hder == NODO_NULO); // la raíz es hoja
	delete(r);
	r = NODO_NULO;
}
...
template <typename T>
int Abin<T>::alturaB(){
	if(this->raizB()==Abin<T>::NODO_NULO){
		return 0;
	}else{
		return alturaBrec(this->raizB(),*this);
	}
}

template <typename T>
int Abin<T>::alturaBrec(typename Abin<T>::nodo n,const Abin<T>& A){
	if(n==Abin<T>::NODO_NULO){
		return 0;
	}else{
		return 1+Max(alturaBrec(A.hijoIzqdoB(n),A),alturaBrec(A.hijoDrchoB(n),A));
	}

}

template<typename T>
int Abin<T>::Max(int a,int b){
	if(a<b){
		return a;
	}else{
		return b;
