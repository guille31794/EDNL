...
template <typename T> class Abin {
    public:
        ...
        ~Abin(); // destructor
    private:
        uint_fast8_t alturaRec(nodo n) const;
        struct celda {
            T elto;
...
template <typename T> inline
uint_fast8_t Abin<T>::profundidad(nodo n) const
{
    if(0 == n)
        return 0;
    else
...
template <typename T> inline
uint_fast8_t Abin<T>::altura(nodo n) const
{
    if(NODO_NULO == n)
        return -1;
    else
        return 1 + max(alturaRec(nodos[n]->hizq), alturaRec(nodos[n]->hder);
}

template <typename T> inline
uint_fast8_t Abin<T>::alturaRec(nodo n) const
{
    NODO_NULO == n ? return 0 : return  ++max(alturaRec(nodos[n]->hizq), alturaRec(nodos[n]->hder);
}
