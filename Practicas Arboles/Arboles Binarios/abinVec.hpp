...
{
public:
typedef int nodo; // índice de la matriz, entre 0 y maxNodos-1
static const nodo NODO_NULO;
explicit Abin(size_t maxNodos); // constructor
void insertarRaizB(const T& e);
void insertarHijoIzqdoB(nodo n,
void insertarHijoDrchoB(nodo n,
void eliminarHijoIzqdoB(nodo n);
void eliminarHijoDrchoB(nodo n);
void eliminarRaizB();
~Abin(); // destructor
bool arbolVacioB() const;
const T& elemento(nodo n) const; // acceso a elto, lectura T& elemento(nodo n); 
const T& e); // acceso a elto, lectura/escritura
const T& e);
nodo raizB() const;
...
Abin<T>& operator =(const Abin<T>& a);  // asignación
private: 
struct celda 
{
   T elto;
...
int maxNodos; // tamaño del vector
int numNodos; // número de nodos del árbol
};
...
{
    assert(numNodos == 0);   // árbol vacío
    numNodos = 1;
    nodos[0].elto = e;
...
{
    assert(n >= 0 && n < numNodos); // nodo válido 
    assert(nodos[n].hizq == NODO_NULO); // n no tiene hijo izqdo. 
    assert(numNodos < maxNodos); // árbol no lleno 
    // añadir el nuevo nodo al final de la secuencia
...
void Abin<T>::eliminarHijoIzqdoB(Abin<T>::nodo n) 
{
    ...
    {
        // Mover el último nodo a la posición del hijo izqdo.
        nodos[hizqdo] = nodos[numNodos-1];
        // Actualizar la posición del hijo (izquierdo o derecho) 
    ...
    // actualizar la posición del padre 
...
void Abin<T>::eliminarHijoDrchoB(Abin<T>::nodo n) 
{
    ...
    {
        // Mover el último nodo a la posición del hijo drcho.
